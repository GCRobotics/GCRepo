...
void L3G::read()
{
  ...
  // Request 192 bytes from gyro or 32 sets of data from gyro
  Serial.println("                Requestiong 192 bytes from gyro");
...
void loop() {
  ...
  if (digitalRead(2))
  {
    ...
    gyro.read();
    if (gyro.g.ReadyToProcessFlag == 1)
    {
      Serial.println("                About to process the data");
      // Performing calculation to all the data points
      // degrees moved = 1/ODR * So *(Reading - R0) = 1/100*.00875*(Reading-5)
      for (int i = 0; i < NUMBER_OF_SETS; i++)
      {
        gyro.g.CurrentAngle_x = gyro.g.CurrentAngle_x + (0.1 * 0.00875 * (gyro.g.x[i] - 40));
        gyro.g.CurrentAngle_y = gyro.g.CurrentAngle_y + (0.01 * 0.00875 * (gyro.g.y[i] - 5));
        gyro.g.CurrentAngle_z = gyro.g.CurrentAngle_z + (0.01 * 0.00875 * (gyro.g.z[i] - 5));
        if (DEBUGGING_MODE)
        {
          Serial.print("               Results of CurrentAngle Calculation for "); Serial.print(i); Serial.println(" :  ");
          Serial.print("g.CurrentAngle_x = "); Serial.print (gyro.g.CurrentAngle_x); Serial.print ("        ");
          Serial.print("g.CurrentAngle_y = "); Serial.print (gyro.g.CurrentAngle_y); Serial.print ("        ");
          Serial.print("g.CurrentAngle_z = "); Serial.print (gyro.g.CurrentAngle_z); Serial.println ("");
        }
      }
      gyro.g.ReadyToProcessFlag = 0;        //Reset Flag
    }
  }
