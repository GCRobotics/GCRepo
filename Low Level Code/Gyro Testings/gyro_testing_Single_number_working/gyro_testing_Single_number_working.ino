...
void setup() {
  ...
  //PCintPort::attachInterrupt(2, burpcount,LOW); // attach a PinChange Interrupt to our pin on the rising edge
  Serial.println("                Printing a set of data!");
}
...
void loop() {
      if (digitalRead(2))
      {
        ...
        for (int i = 0; i < NUMBER_OF_SETS; i++)
        {
          Serial.print (gyro.g.x[i]); Serial.print(",");
          Serial.print (gyro.g.y[i]); Serial.print(",");
          Serial.print (gyro.g.z[i]); Serial.println(",");
        }
      }
}
